...
// Helper to calculate the rich forecast data
const calculateForecastData = async (userId) => {
  ...
  // FIX: Declare undefined variables
  let forecastNextMonth = 0;
  let forecast3Months = 0;
  let forecast6Months = 0;
  ...
  if (totalReduction > 0) {
    ...
    forecastNextMonth = Math.max(0, forecastNextMonth - totalReduction);
    forecast3Months = Math.max(0, forecast3Months - (totalReduction * 3));
    forecast6Months = Math.max(0, forecast6Months - (totalReduction * 6));
...
const getUnifiedForecast = async (req, res) => {
  try {
    ...
    // First, check if a Forecast record exists for this user
    const existingForecast = await Forecast.findOne({ userId });
    
    if (!existingForecast) {
      return res.status(404).json({
        success: false,
        message: 'No forecast exists. Please generate your first forecast.',
        needsGeneration: true
      });
    }

    // If it exists, calculate the latest rich data
    const data = await calculateForecastData(userId);
...
const generateForecast = async (req, res) => {
  try {
    ...
    // Save to Forecast collection
    await Forecast.findOneAndUpdate(
      { userId },
      {
        userId,
        month: currentMonthKey,
        currentCarbon: data.currentMonth,
        predictedCarbon: data.forecastNextMonth,
        trend: data.trendDirection.toLowerCase(),
        // Just mock a reduction percentage for the record based on trend
        reductionPercentage: data.trendDirection === 'Decreasing' ? 5 : 0 
      },
    ...
    console.error(`Error in generateForecast: ${error.message}`);
    return res.status(500).json({ success: false, message: 'Server Error' });
  }
};
